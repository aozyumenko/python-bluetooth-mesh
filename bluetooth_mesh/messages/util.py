...
class Opcode(Construct):
    ...
    def _build(self, obj, stream, context, path):
        if obj > 0xFFFF:
            stream_write(stream, obj.to_bytes(3, byteorder="big"), 3)
        elif obj > 0xFF:
            stream_write(stream, obj.to_bytes(2, byteorder="big"), 2)
        else:
            stream_write(stream, obj.to_bytes(1, byteorder="big"), 1)
