...
_opcodes = {key: opcode for opcode_class in OPCODES for key, opcode in opcode_class._value2member_map_.items()}
...
class SampleApplication(Application):
    ...
    async def descriptor_get(self, addr, app_index, arguments):
        def receive_status(
            ...
            opcode = _opcodes[message.opcode].name.lower() if message.opcode in _opcodes else message.opcode
            print(opcode)
            print(message[opcode])

        client = self.elements[0][SensorClient]
        client.app_message_callbacks[SensorOpcode.SENSOR_DESCRIPTOR_STATUS].add(receive_status)
